...
import { getReportMBR, getReportFSTree, getReportJournal } from '@/lib/api'

export default function ReportsPage() {
  const [id, setId] = useState('')
  const [path, setPath] = useState('/')
  const [tab, setTab] = useState<'mbr'|'tree'|'journal'>('mbr')
  ...
  return (
    <section className="bg-white rounded-2xl shadow-sm border p-3">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <input className="px-3 py-2 rounded-lg border" placeholder="ID montado (vdXXXX)" value={id} onChange={e=>setId(e.target.value)} />
        {tab==='tree' && (
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <TabBtn active={tab==='tree'} onClick={()=>setTab('tree')}>FS Tree</TabBtn>
